...
def main():
	...
	if train:
		...
		trainset = torchvision.datasets.ImageFolder('E:\\1_task\CNN\CNN_train', transform=transform)
		...
		torch.save(net.state_dict(), "E:\\1_task\\CNN\\examples\\cancer_classification.model")
	else:
		net.load_state_dict(torch.load("E:\\1_task\\CNN\\examples\\1\\cancer_classification.model"))
		# Adadelta,Adagrad,AdamW,SparseAdam?,Adamax,ASGD,LBFGS?,NAdam?,RAdam,RMSprop,Rprop
		
		net.eval()

		testset = torchvision.datasets.ImageFolder('E:\\1_task\CNN\CNN_Matlab_test', transform=transform)
		...
		np.save("E:\\1_task\CNN\\CNN_Matlab_test\labels", np.concatenate(all_labels))  
		np.savetxt("E:\\1_task\CNN\\CNN_Matlab_test\\test_results.txt", np.concatenate(all_predictions))
